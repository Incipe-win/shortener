...
var (
	...
	cacheSqlTestSequenceIdPrefix   = "cache:sqlTest:sequence:id:"
	cacheSqlTestSequenceStubPrefix = "cache:sqlTest:sequence:stub:"
)
...
func (m *defaultSequenceModel) Delete(ctx context.Context, id uint64) error {
	...
	sqlTestSequenceIdKey := fmt.Sprintf("%s%v", cacheSqlTestSequenceIdPrefix, id)
...
func (m *defaultSequenceModel) FindOne(ctx context.Context, id uint64) (*Sequence, error) {
	sqlTestSequenceIdKey := fmt.Sprintf("%s%v", cacheSqlTestSequenceIdPrefix, id)
	var resp Sequence
	err := m.QueryRowCtx(ctx, &resp, sqlTestSequenceIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultSequenceModel) FindOneByStub(ctx context.Context, stub string) (*Sequence, error) {
	sqlTestSequenceStubKey := fmt.Sprintf("%s%v", cacheSqlTestSequenceStubPrefix, stub)
	var resp Sequence
	err := m.QueryRowIndexCtx(ctx, &resp, sqlTestSequenceStubKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v any) (i any, e error) {
...
func (m *defaultSequenceModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheSqlTestSequenceIdPrefix, primary)
}
